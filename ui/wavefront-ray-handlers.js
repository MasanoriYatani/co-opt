...
(async () => {
    try {
        ...
        console.log('✅ Brent法のインポート成功:', typeof brentMethod);
    } catch (error) {
        console.warn('⚠️ Brent法のインポートに失敗、フォールバック使用:', error.message);
    }
})();
...
export function setupWavefrontRayButtons() {
    console.log('🎯 波面収差光線描画ボタンをセットアップ中...');
    ...
    if (drawBtn) {
        drawBtn.addEventListener('click', async function() {
            console.log('🎯 [DrawWavefrontRays] OPD光線描画ボタンがクリックされました');
            await handleDrawWavefrontRays();
        });
        console.log('✅ Draw OPD Rays ボタンにイベントハンドラーを追加');
    } else {
        console.warn('⚠️ Draw OPD Rays ボタンが見つかりません');
    }
    
    if (clearBtn) {
        clearBtn.addEventListener('click', function() {
            console.log('🧹 [ClearWavefrontRays] OPD光線クリアボタンがクリックされました');
            handleClearWavefrontRays();
        });
        console.log('✅ Clear OPD Rays ボタンにイベントハンドラーを追加');
    } else {
        console.warn('⚠️ Clear OPD Rays ボタンが見つかりません');
    }
}
...
async function handleDrawWavefrontRays() {
    try {
        ...
        console.log(`  光学系面数: ${opticalSystemRows ? opticalSystemRows.length : 0}`);
        console.log(`  Objectデータ数: ${objectRows ? objectRows.length : 0}`);
        
        // Objectデータの詳細を表示
        if (objectRows && objectRows.length > 0) {
            console.log('🔍 Objectデータ詳細:');
        ...
        // 光学系データを取得
        console.log('🔍 光学系データ取得中...');
        console.log('🔍 光学系データ:', opticalSystemRows ? `${opticalSystemRows.length}面` : 'null');
        ...
        // 波面収差解析用のフィールド設定を取得
        console.log('🔍 フィールド設定取得中...');
        const fieldSetting = getCurrentFieldSetting();
        console.log('🔍 フィールド設定:', fieldSetting);
        
        // 🔍 フィールド設定の詳細ログ
        console.log('🔍 詳細フィールド設定:');
        console.log(`  フィールドタイプ: ${fieldSetting.fieldType || '未設定'}`);
        if (fieldSetting.fieldAngle) {
            console.log(`  画角: X=${fieldSetting.fieldAngle.x}°, Y=${fieldSetting.fieldAngle.y}°`);
        }
        if (fieldSetting.xHeight !== undefined || fieldSetting.yHeight !== undefined) {
            console.log(`  高さ: X=${fieldSetting.xHeight}mm, Y=${fieldSetting.yHeight}mm`);
        }
        console.log(`  表示名: ${fieldSetting.displayName}`);
        
        if (!fieldSetting) {
            console.error('❌ フィールド設定が取得できません');
            alert('フィールド設定が取得できません。');
            return;
        }
        
        console.log('📊 使用するフィールド設定:', fieldSetting);

        // 主波長（n(λ)）
        const wavelength = window.getPrimaryWavelengthForAberration ? window.getPrimaryWavelengthForAberration() : 0.55;
        
        // 波面収差解析器を作成（windowオブジェクト経由）
        console.log('🔍 波面収差解析器作成中...');
        ...
        console.log('🔍 解析器:', analyzer ? '作成成功' : '作成失敗');
        
        if (!analyzer) {
            console.error('❌ 波面収差解析器の初期化に失敗');
            alert('波面収差解析器の初期化に失敗しました。');
            return;
        }
        ...
        console.log(`🌊 波面収差マップ生成中 (gridSize=${gridSize})...`);
        
        // 🆕 適切な絞りビネッティングを考慮した波面収差マップを生成
        console.log('🌊 絞り考慮波面収差マップ生成中...');
        ...
        console.log('🔍 基本波面収差マップ生成結果:', baseWavefrontMap ? '成功' : '失敗');
        
        if (!baseWavefrontMap) {
            console.error('❌ 基本波面収差マップ生成に失敗');
            alert('波面収差計算に失敗しました。');
            return;
        }
        ...
        console.log('🔍 絞り適用後の光線データ:', wavefrontMap?.rayData ? `${wavefrontMap.rayData.length}本` : 'なし');
        ...
        console.log('⚠️ 注意: ベストエフォート周辺光線は一時的に無効化されています');
        
        if (!wavefrontMap || !wavefrontMap.rayData) {
            console.error('❌ 絞り適用後の波面収差計算に失敗');
            alert('波面収差計算に失敗しました。');
            return;
        }
        
        console.log(`📊 生成された光線データ: ${wavefrontMap.rayData.length}本`);
        
        // 🔍 光線データ構造の詳細デバッグ
        if (wavefrontMap.rayData.length > 0) {
            const firstRay = wavefrontMap.rayData[0];
            console.log(`🔍 最初の光線データ構造:`, firstRay);
            console.log(`🔍 光線データのキー:`, Object.keys(firstRay));
            if (firstRay.ray) {
                console.log(`🔍 ray プロパティ:`, typeof firstRay.ray, firstRay.ray);
                console.log(`🔍 ray のキー:`, Object.keys(firstRay.ray));
                if (firstRay.ray.path) {
                    console.log(`🔍 ray.path:`, Array.isArray(firstRay.ray.path), firstRay.ray.path?.length);
                }
            }
        }
        
        // 光線データの詳細をチェック（改善されたフィルタリング）
        ...
        console.log(`🔍 有効光線: ${validRays.length}本, ケラレ光線: ${vignettedRays.length}本`);
        
        if (validRays.length === 0) {
            console.warn('⚠️ 有効な光線データがありません');
            alert('有効な光線データがありません。光学系設定を確認してください。');
            return;
        }
        
        // 描画オプションを設定
        console.log('🔍 3Dシーン確認:', targetScene ? '存在' : '未定義', {
            usingPopup: !!(window.popup3DWindow && !window.popup3DWindow.closed && window.popup3DWindow.scene)
        });
        const drawOptions = {
            scene: targetScene,
            showVignetted: true,
            colorMode: 'opd', // OPD値による色分け
            opdRange: null // 自動範囲設定
        };
        
        // 🔧 **重要修正**: 描画前に既存光線を完全クリア
        console.log('🧹 描画前クリア: 既存の全光線を削除中...');
        if (!targetScene) {
            console.error('❌ 3Dシーンが見つかりません（Renderポップアップを開いてください）');
            alert('3D描画シーンが見つかりません。先に Render を開いてください。');
            return;
        }
        ...
        console.log('✅ 波面収差光線描画完了');
        alert(`波面収差光線を描画しました。\n光線数: ${wavefrontMap.rayData.length}本\n有効光線: ${validRays.length}本`);
        
    } catch (error) {
        console.error('❌ 波面収差光線描画エラー:', error);
        alert(`波面収差光線描画エラー: ${error.message}`);
    }
}
...
async function applyApertureVignetting(baseWavefrontMap, opticalSystemRows) {
    ...
    try {
        ...
        try {
            ...
            console.warn(`⚠️ 絞り半径推定エラー: ${error.message}, デフォルト値使用`);
        }
        ...
        for (const direction of marginalDirections) {
            ...
            if (bestEffortResult.success || bestEffortResult.bestError < apertureRadius * 0.7) {
                ...
                try {
                    ...
                    console.warn(`  ⚠️ ${direction.name}: 光線追跡エラー: ${error.message}`);
                }
            } else {
        ...
        if (bestEffortRays.length === 0) {
            ...
            for (const offset of simpleDirections) {
                try {
                    ...
                    console.warn(`  ⚠️ ${offset.name}: 簡易周辺光線生成失敗: ${error.message}`);
                }
            }
    ...
    try {
        ...
        if (brentMethod && typeof brentMethod === 'function') {
            ...
            try {
                ...
                console.warn(`⚠️ [Brent] X方向最適化失敗: ${error.message}, フォールバック値使用`);
                optimalDeltaX = 0;
            }
            ...
            try {
                ...
                console.warn(`⚠️ [Brent] Y方向最適化失敗: ${error.message}, フォールバック値使用`);
                optimalDeltaY = 0;
            }
        } else {
        ...
        console.error(`❌ [Brent] 最適化エラー: ${error.message}`);
